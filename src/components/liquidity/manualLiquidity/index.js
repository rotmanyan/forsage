...
const setUserCurrency = value => ({
  type: ACTIONS.SET_USER_CURRENCY,
  payload: value,
})
const setTargetCurrency = value => ({
  type: ACTIONS.SET_TARGET_CURRENCY,
  payload: value,
})
const setTransactionStatus = value => ({
...
const ManualLiquidity = ({ setMode }) => {
  ...
  useEffect(() => {
    if (!USER_CURRENCIES.includes(state.userCurrency)) {
      const newUserCurrency = USER_CURRENCIES.find(
        ({ label }) => label === state.userCurrency.label
      )
      dispatch(setUserCurrency(newUserCurrency || USER_CURRENCIES[0]))
    }
    if (!CURRENCIES.includes(state.targetCurrency)) {
      const newTargetCurrency = CURRENCIES.find(
        ({ label }) => label === state.targetCurrency.label
      )
    ...
    >
      <Container>
        <ContentWrapper alignItems="center" column>
          <Progress
            dots={[...Object.values(MANUAL_PROGRESS)]}
            titles={MANUAL_PROGRESS_TITLES}
            step={progress}
            setStep={nextHandler}
          />
...
export default ManualLiquidity
